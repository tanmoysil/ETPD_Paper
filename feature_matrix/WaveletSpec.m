...
if any(strcmp(varargin, 'WAVELETSPEC'))
    ...
    wType = 'morlet';
    wParams = repmat(10,size(freqs)); %%width of the wavelet
end
...
for j = 1:length(freqs)
    ...
    switch wType
        ...
        case 'mexican_hat'
            % Define parameters for the Mexican Hat wavelet
            sigma = wParams / sqrt(3);
            amp = sqrt(2 / (3 * sigma * sqrt(pi)));
            tSupp = (-wParams/currFreq):tStep:(wParams/currFreq);
            psi = amp * (1 - (tSupp.^2 / sigma^2)) .* exp(-(tSupp.^2) / (2 * sigma^2));
            w1 = real(psi);
            w2 = imag(psi);
        case 'haar'
            % Define parameters for the Haar wavelet
            tSupp = (-wParams/currFreq):tStep:(wParams/currFreq);
            psi = zeros(size(tSupp));
            psi(tSupp >= 0 & tSupp < 0.5) = 1;
            psi(tSupp >= 0.5 & tSupp < 1) = -1;
            psi = psi / sqrt(tStep);
            w1 = psi;
            w2 = zeros(size(psi)); % Imaginary part is zero for Haar wavelet
        case 'db4' % Daubechies 4 Wavelet
            [psi, tSupp] = wfilters('db4');
            psi = psi / sqrt(tStep); % Normalization
            w1 = psi;
            w2 = zeros(size(psi)); % Imaginary part is zero for Daubechies wavelets
        case 'sym4' % Symlet 4 Wavelet
            [psi, tSupp] = wfilters('sym4');
            psi = psi / sqrt(tStep); % Normalization
            w1 = psi;
            w2 = zeros(size(psi)); % Imaginary part is zero for Symlet wavelets
        case 'bior2.2' % Biorthogonal 2.2 Wavelet
            [psi, tSupp] = wfilters('bior2.2');
            psi = psi / sqrt(tStep); % Normalization
            w1 = psi;
            w2 = zeros(size(psi)); % Imaginary part is zero for Biorthogonal wavelets
        case 'coif5' % Coiflet 5 Wavelet
            [psi, tSupp] = wfilters('coif5');
            psi = psi / sqrt(tStep); % Normalization
            w1 = psi;
            w2 = zeros(size(psi)); % Imaginary part is zero for Coiflet wavelets
        case 'dmey' % Discrete Meyer Wavelet
            [psi, tSupp] = wfilters('dmey');
            psi = psi / sqrt(tStep); % Normalization
            w1 = psi;
            w2 = zeros(size(psi)); % Imaginary part is zero for Discrete Meyer wavelet
        case 'gaus1' % Gaussian Wavelet (1st derivative of Gaussian)
            tSupp = (-wParams/currFreq):tStep:(wParams/currFreq);
            psi = -tSupp .* exp(-0.5 * (tSupp.^2));
            psi = psi / sqrt(tStep); % Normalization
            w1 = psi;
            w2 = zeros(size(psi)); % Imaginary part is zero for Gaussian wavelet
        case 'shan1' % Shannon Wavelet (sinc function)
            tSupp = (-wParams/currFreq):tStep:(wParams/currFreq);
            psi = sinc(tSupp * pi);
            psi = psi / sqrt(tStep); % Normalization
            w1 = psi;
            w2 = zeros(size(psi)); % Imaginary part is zero for Shannon wavelet
        otherwise
